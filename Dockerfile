...
RUN \
  ...
  && mkdir -p /tmp/rar \
  && curl -L -o /tmp/rar/rarlinux.tar.gz https://rarlab.com/rar/rarlinux-x64-5.5.0.tar.gz \
  ...
  && rm -Rf /tmp/rar \
  && curl -V && wget -V | head -n 4 && unrar | head -n 2 | tail -n 1 && nginx -v \
  && apt-get -yq autoremove && apt-get -yq clean && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
...
RUN \
  mkdir -pv /data/mirror /data/keys /data/logs \
  && /app/nod32-mirror.sh -c \
  && mv /docker/docker-entrypoint.sh /docker-entrypoint.sh \
  && chmod +x /docker-entrypoint.sh \
  && rm -Rf /var/www && mv /docker/www /var/www \
